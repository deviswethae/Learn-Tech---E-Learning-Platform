...
export default function Payment({ isOpen, onClose }) {
  ...
  function handlePayment() {
    if (!token) {
      showToast({ toast, message: "User not logged in!", color: "red" });
      return;
    }
    if (!courseId) {
      showToast({ toast, message: "Invalid course!", color: "red" });
      return;
    }
            ...
            <Box>
              <Box>
                ...
                <Box>
                  ...
                  <Box>
                    <Text fontSize="12px">
                    LearnTech is required by law to collect applicable transaction
